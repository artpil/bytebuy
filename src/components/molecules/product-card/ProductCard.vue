...
const formatPrice = (price, currency = 'USD') => {
  ...
  // Для рубля нужна ru-RU локаль (₽ после числа, "1 140 ₽"),
  // для остальных валют — en-US (символ перед числом, "$19.90")
  ...
  try {
    ...
    // На случай неизвестного/некорректного кода валюты в фиде —
    // не роняем страницу, просто показываем число с кодом как есть
    return `${price} ${code}`
  }
}
